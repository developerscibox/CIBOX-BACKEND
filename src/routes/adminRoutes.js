...
import { getRelayMetrics } from "../controllers/relayMetricsController.js";
...
router.get("/relay-metrics", requirePermission(PERMISSIONS.REPORTS_READ), getRelayMetrics);
router.get("/actividad", requirePermission(PERMISSIONS.REPORTS_READ), getActividad);
router.get("/desempeno", requirePermission(PERMISSIONS.REPORTS_READ), getDesempeno);
...
// Reseed demo: bloqueado en producción (contamina ventas/finanzas reales),
// salvo override explícito con ALLOW_DEMO_RESEED=true.
const blockReseedInProd = (req, res, next) => {
  if (process.env.NODE_ENV === "production" && process.env.ALLOW_DEMO_RESEED !== "true") {
    return next(new ForbiddenError("Reseed demo deshabilitado en producción"));
  }
  next();
};
router.post("/demo/reseed", requirePermission(PERMISSIONS.USERS_MANAGE), blockReseedInProd, reseedDemo);
router.post("/demo/seed-all", requirePermission(PERMISSIONS.USERS_MANAGE), reseedDemoAll);
// Seed de fichas (demo comercial): historia de 30 días + lotes + mínimos,
// coherente entre reportes y fichas. Solo admin; permitido en prod porque
// ESTA instalación es la demo de venta. Re-ejecutable (limpia lo suyo).
router.post("/demo/seed-ficha", requirePermission(PERMISSIONS.USERS_MANAGE), async (req, res, next) => {
  try {
    const { seedFichaDemo } = await import("../scripts/seedFichaDemo.js");
    const out = await seedFichaDemo();
